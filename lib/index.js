...
fastify.post('/scrape', (req, reply) => {
    ...
    if (b && typeof b["url"] === "string") {
        ...
        reply.send(scrapeError('Missing URL string parameter.', req));
    }
});

fastify.post('/request_preview', (req, reply) => {
    const b = req.body;
    if (b && typeof b["url"] === "string" && 
        typeof b["serviceUrl"] === "string") {
        const rawServiceUrl = b["serviceUrl"].trim();
        const rawTargetUrl = b["url"].trim();
        let suo;
        let uo;
        try {
            // Validates the service URL a little.
            suo = new URL(rawServiceUrl);
        } catch (e) {
            reply.send(scrapeError(`Invalid service URL "${rawServiceUrl}".`,
                req, `\n ${e}`));
            return;
        }
        try {
            // Validates the target URL a little.
            uo = new URL(rawTargetUrl);
        } catch (e) {
            reply.send(scrapeError(`Invalid URL "${rawTargetUrl}".`, req, 
                `\n ${e}`));
            return;
        }
        const serviceUrl = suo.toString();
        const targetUrl = uo.toString();
        request.post({url: serviceUrl, json: {url: targetUrl}},
            (err, res, body) => {
            if (err) { 
                const ae = addressError(err, req);
                if (ae) {
                ...
                // Relay the URL back to the client for it to know which
                // URL this request belongs to.
                body["_url"] = b["url"];
                reply.send(body);
            } else {
                reply.send(scrapeError(
        ...
        reply.send(scrapeError('Missing either the service URL or the ' +
            'target URL string parameter.', req));
    }
});
...
// Página de interface simples para o teste de URL preview.
fastify.get('/url_preview', (req, reply) => {
    reply.view('/templates/url_preview.ejs')
});
