...
fastify.post('/scrape', (req, reply) => {
    ...
    if (b && b["url"]) {
        ...
        try {
            ...
            req.log.error(`Error: Invalid URL "${b["url"]}"\n ${e}`);
            return;
        }
        const url = uo.toString();
        request(url, (err, res, body) => {
            if (err) { 
                req.log.error(`Error. Request returned the error ${err}.`);
            } else if (res.statusCode === 200) {
                const mode = b["mode"];
                if (mode && mode !== "parse5" && mode !== "cheerio" && 
                    mode !== "loose") {
                    req.log.error(`Error. Unknown scrape mode "${mode}".`);
                ...
                req.log.error("Error. Could not process the server response " +
