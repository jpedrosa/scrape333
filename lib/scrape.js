...
function joinUri(a, b) {
    const aGotSlash = a.endsWith("/");
    const bGotSlash = b.startsWith("/");
    if (aGotSlash && bGotSlash) {
        return `${a}${b.slice(1)}`;
    }
    return (aGotSlash || bGotSlash) ? `${a}${b}` : `${a}/${b}`;
}
...
function scrape(s, url, mode) {
    ...
    let ogimg = results["og:image"];
    if (ogimg) {
        ogimg = ogimg.trim();
        // Testa se o URL da imagem inclui http:// https:// file:/// e se
        // começa com ponto (.) ou barra (/) em caso de URL relativo. Se
        // precisar a gente adiciona a parte inicial do URL.
        if (!reURLStart.test(ogimg)) {
            if (ogimg.startsWith(".")) {
                ogimg = joinUri(url, ogimg);
            } else if (ogimg.startsWith("/")) {
                try {
                    // Extrair a parte sem arquivos do URL.
                    // http://esta.parte.com/ignora/o/resto
                    // A gente só quer http://esta.parte.com/
                    ogimg = joinUri(new URL(url).origin, ogimg);
                } catch (e) {
                    console.error(`Error: Unable to parse this url "${url}"`);
                }
            }
        }
    ...
    // Só para desencargo de consciência, a gente checa se algum title foi
    // achado. Caso contrário a gente coloca um vazio mesmo porque de acordo
    // com o desafio este é um campo obrigatório.
    if (!all_results["title"]) {
        all_results["title"] = "";
    }
    return all_results;
}
